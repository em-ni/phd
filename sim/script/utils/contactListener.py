...
class ContactListener(Sofa.Core.Controller):
    ...
    def add_drawNode(self):
        ...
        MOdraw = drawNode.addObject(
            "MechanicalObject",
            name="drawPositions",
            position="@/root_node/{self.object_name}/{self.collision_name}/MechanicalObject.position",
            size=8,
        )
        drawNode.addObject(
            "ConstantForceField",
    ...
    def onAnimateEndEvent(self, event):
        ...
        constraint = (
            self.root_node.getChild(self.object_name)
            .getChild(self.collision_name)
            .MechanicalObject.constraint.value
        )
        # Parse constraint data (needed to pass from v22.12 to v23.12)
        constraint = parse_constraint_data(constraint)
        if self.debug:
            print(f"ContactListener: Constraint: {constraint}")
        if self.debug:
            print(
                f"ContactListener: type(constraint): {type(constraint)}, len(constraint): {len(constraint)}"
            )
        ...
        if self.debug:
            print(f"ContactListener: constraintMatrixInline: {constraintMatrixInline}")
        ...
        while index < len(constraintMatrixInline):
            if self.debug:
                print(f"ContactListener: index: {index}")
            nbConstraint = int(constraintMatrixInline[index + 1])
            currConstraintID = int(constraintMatrixInline[index])
            if self.debug:
                print(
                    f"ContactListener: nbConstraint: {nbConstraint}, currConstraintID: {currConstraintID}"
                )
            for pts in range(nbConstraint):
                ...
                constraintDirections.append(
                    [
                        constraintMatrixInline[currIDX + 1],
                ...
                if self.debug:
                    print(
                        f"ContactListener: pointId: {pointId}, constraintId: {constraintId}, constraintDirections: {constraintDirections}"
                    )
        ...
        nbDofs = len(
            self.root_node.getChild(self.object_name)
            .getChild(self.collision_name)
            .MechanicalObject.position.value
        )
        ...
        if self.debug:
            print(f"ContactListener: forcesNorm size: {len(forcesNorm)}")
        ...
        for i in range(len(pointId)):
            ...
            if i < len(constraintDirections) and constraintId[i] < len(forcesNorm):
                if indice < nbDofs:
                    forces[indice][0] += (
                        constraintDirections[i][0] * forcesNorm[constraintId[i]] / dt
                    )
                    forces[indice][1] += (
                        constraintDirections[i][1] * forcesNorm[constraintId[i]] / dt
                    )
                    forces[indice][2] += (
                        constraintDirections[i][2] * forcesNorm[constraintId[i]] / dt
                    )
                else:
                    if self.debug:
                        print(
                            f"ContactListener: Warning: indice {indice} out of bounds for forces array with size {nbDofs}"
                        )
            else:
                if self.debug:
                    print(
                        f"ContactListener: Warning: Access out of bounds. i: {i}, constraintId[i]: {constraintId[i]}, len(constraintDirections): {len(constraintDirections)}, len(forcesNorm): {len(forcesNorm)}"
                    )
        ...
        if len(constraintMatrixInline) > 0:
            if self.debug:
                print(f"ContactListener: indices: {list(range(0,nbDofs,1))}")
            if self.debug:
                print(f"ContactListener: forces: {forces}")
            if self.debug:
                print(
                    f"ContactListener: position: {self.root_node.getChild(self.object_name).getChild(self.collision_name).MechanicalObject.position.value}"
                )
...
def parse_constraint_data(constraint_str):
    ...
    pattern = re.compile(
        r"Constraint ID : (\d+) +dof ID : (\d+) +value : ([\-\d\.e]+) ([\-\d\.e]+) ([\-\d\.e]+)"
    )
    ...
    for match in matches:
        ...
        constraints[int(constraint_id)].append(
            (int(dof_id), [float(val) for val in values])
        )
