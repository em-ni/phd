...
Rc = 0.75*1e-3;  
Rh = 0.4*1e-3;  
Rs_nominal = 0.3*1e-3; % nominal value not stretched
    
% Inner radii
Rci = 0.6*1e-3;  
Rhi = 0.3*1e-3;   
Rsi_nominal = 0.15*1e-3;  % nominal value not stretched

% Silicon tube (estimated value after stretching) (only used for comparison with circular cross section)
Rs = (2*Rci - 2*Rh) / 2;
Rsi = 0.1*1e-3; % Can be modified
...
% Young's modulus
Es = 1.648e6; % [Pa]
spring_c = 0.035*1e3; % [N/m]
Ec = spring_c*L/Ac;
spring_h = 0.035*1e3; % [N/m]
Eh = spring_h*L/Ah; 
...
load("data/free_motion/linear_model_p_vs_k.mat", "mdl");
...
a_max = sqrt( 2*Rci*b - b^2 ); % Form geometric considerations, maximum inscribed rhombus in the space between hollow channle and external coil (not super precise tbh)
a_min = b; % a circle in the inferior limit
ai_min = b;
...
for i = 1:size(X0,1)
    ...
    error = sqrt( mean( (k_val - k_data)^2 + (epsilon_val - epsilon_data)^2) )  / (max([k_data; epsilon_data]) - min([k_data; epsilon_data]));
...
disp('Ranked results (a [mm], ai [mm], bi [mm], calculated k, calculated epsilon, NRMSE):');
% Loop over the sorted results and display each result
for i = 1:size(sorted_results, 1)
    fprintf('a = %.3f*1e-3, ai = %.3f*1e-3, bi = %.3f*1e-3, h = %.3f*1e-3, k = %.3f, epsilon = %.3f, NRMSE = %.3e\n', ...
...
save('data/free_motion/optimization_results.mat', 'epsilon_opt', 'k_opt');
...
fprintf('Curvature from data k_data = %d* p\n', k_data);

calculate_epsilon(a, ai, bi, Ac, Ah, Es, Ec, Eh, Rci, Rh, P, 1);
fprintf('Strain from data epsilon_data = %d* p\n', epsilon_data);

% Print the best results with final error
disp('Best results (parameter displayed in mm):');
fprintf('a = %.3f*1e-3, ai = %.3f*1e-3, bi = %.3f*1e-3, h = %.3f*1e-3, k = %f, epsilon = %f, NRMSE = %.3e\n', ...
...
function k = calculate_k(a, ai, bi, h, Ac, Ah, Es, Ec, Eh, Rc, Rh, Rci, Rhi, P, save)
    ...
    k = (M / (Es*Is + Ec*Ic + Eh*Ih)) ;

    if save
        % Print statements
        fprintf('\nPressure is in Pa\nCurvature is in m^-1\n');
        fprintf('Input torque M(p) = %d * p\n', M/P);
        fprintf('Elastic term K(k) = %d * k\n', (Es*Is + Ec*Ic + Eh*Ih));
        fprintf('Curvature from opt k_opt = %d * p\n', k);        

        % Save the functions to .m files
        syms p k 'real';

        matlabFunction(M*p, 'Vars', p , 'File', ['functions/','input_torque'], 'Optimize', true);
        matlabFunction((Es*Is + Ec*Ic + Eh*Ih)*k, 'Vars', k , 'File', ['functions/','elastic_term'], 'Optimize', true);
...
function epsilon = calculate_epsilon(a, ai, bi, Ac, Ah, Es, Ec, Eh, Rci, Rh, P, save)
    b = (2*Rci - 2*Rh)/2;
    As = pi*(a*b - ai*bi);
    Ap = pi*(ai*bi);
    ...
    if save
        ...
        fprintf('Input force F(p) = %d * p\n', Fp/P);
        fprintf('Elastic term K(epsilon) = %d * epsilon\n', (Es*As + Ec*Ac + Eh*Ah));
        fprintf('Strain from opt epsilon_opt = %d * p\n', epsilon);
        
        % Save the functions to .m files
        syms p epsilon 'real';

        matlabFunction(Fp*p, 'Vars', p , 'File', ['functions/','input_force'], 'Optimize', true);
        matlabFunction((Es*As + Ec*Ac + Eh*Ah)*epsilon, 'Vars', epsilon , 'File', ['functions/','elastic_term_epsilon'], 'Optimize', true);
    end
end
