...
def spec(N):
    ...
    return (
        np.round(np.clip(np.stack([-t, 510 - np.abs(t), t], axis=1), 0, 255)).astype(
            "float32"
        )
        / 255
    )
...
def collect_odometry_error_per_dataset(dataset_multierror_list, dataset_names):
    ...
    for dataset_idx, dataset_nm in enumerate(dataset_names):
        ...
        for d in dataset_mt_error:
            assert (
                cur_res["subtraj_len"] == d.rel_distances
            ), "inconsistent boxplot distances"
        print("Using distances {0} for relative error.".format(cur_res["subtraj_len"]))
        ...
        print("< Finish processing {0} for all configurations.".format(dataset_nm))
...
def plot_odometry_error_per_dataset(
    dataset_rel_err, dataset_names, algorithm_names, datasets_outdir, plot_settings
):
    for dataset_idx, dataset_nm in enumerate(dataset_names):
        ...
        ax = fig.add_subplot(
            121, xlabel="Distance traveled (m)", ylabel="Translation error (\%)"
        )
        pu.boxplot_compare(
            ax,
            distances,
            [rel_err["trans_err_perc"][v] for v in algorithm_names],
            config_labels,
            config_colors,
            legend=False,
        )
        ...
        pu.boxplot_compare(
            ax,
            distances,
            [rel_err["rot_deg_per_m"][v] for v in algorithm_names],
            config_labels,
            config_colors,
            legend=True,
        )
        fig.tight_layout()
        fig.savefig(
            output_dir + "/" + dataset_nm + "_trans_rot_error" + FORMAT,
            bbox_inches="tight",
            dpi=args.dpi,
        )
        plt.close(fig)
...
def plot_rmse_per_dataset(
    algorithm_rmse, dataset_names, algorithm_names, output_dir, plot_settings
):
    config_labels = []
    config_colors = []
    ...
    labels = [
        plot_settings["datasets_labels"][v].replace("_", r"\_") for v in dataset_names
    ]
    ...
    fig.tight_layout()
    fig.savefig(
        output_dir + "/" + "all_translation_rmse" + FORMAT,
        bbox_inches="tight",
        dpi=args.dpi,
    )
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(
        output_dir + "/" + "all_rotation_rmse" + FORMAT,
        bbox_inches="tight",
        dpi=args.dpi,
    )
    plt.close(fig)
...
def plot_trajectories(
    ...
    for dataset_idx, dataset_nm in enumerate(dataset_names):
        ...
        for alg in algorithm_names:
            if plot_traj_per_alg:
                ...
                ax_i = fig_i.add_subplot(
                    111, aspect="equal", xlabel="x [m]", ylabel="y [m]"
                )
                pu.plot_trajectory_top(
                    ax_i,
                    p_es_0[alg],
                    "b",
                    "Estimate " + plot_settings["algo_labels"][alg],
                    0.5,
                )
        ...
        fig.tight_layout()
        fig.savefig(
            output_dir + "/" + dataset_nm + "_trajectory_top" + FORMAT,
            bbox_inches="tight",
            dpi=args.dpi,
        )
        plt.close(fig)
        ...
        fig.tight_layout()
        fig.savefig(
            output_dir + "/" + dataset_nm + "_trajectory_side" + FORMAT,
            bbox_inches="tight",
            dpi=args.dpi,
        )
        plt.close(fig)
...
def plot_overall_odometry_errors(
    odo_err_col, algorithm_names, rel_e_distances, plot_settings, output_dir
):
    for et in odo_err_col:
        ...
        fig = plt.figure(figsize=(12, 3))
        ax = fig.add_subplot(111, xlabel="Distance traveled [m]", ylabel=ylabel)
        pu.boxplot_compare(ax, distances, errors, labels, colors, legend=True)
        fig.tight_layout()
        fig.savefig(
            output_dir + "/" + "overall_{}".format(et) + FORMAT,
            bbox_inches="tight",
            dpi=args.dpi,
        )
        plt.close(fig)
...
def parse_config_file(config_fn, sort_names):
    ...
    if boxplot_distances and boxplot_percentages:
        print(
            Fore.RED + "Found both both distances and percentages for boxplot distances"
        )
...
if __name__ == "__main__":
    ...
    config_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "../analyze_trajectories_config"
    )

    parser.add_argument(
        "config", type=str, help="yaml file specifying algorithms and datasets"
    )
    parser.add_argument(
        "--output_dir", help="Folder to output plots and data", default=default_path
    )
    parser.add_argument(
        "--results_dir",
    ...
    parser.add_argument(
        "--platform", help="HW platform: [laptop, nuc, odroid, up]", default="laptop"
    )
    parser.add_argument(
        "--mul_trials",
        type=int,
    ...
    # RMSE (ATE)
    parser.add_argument(
        "--rmse_table", help="Output rms erros into latex tables", action="store_true"
    )
    parser.add_argument(
        "--rmse_table_median_only", action="store_true", dest="rmse_median_only"
    )
    parser.add_argument(
        "--rmse_boxplot", help="Plot the trajectories", action="store_true"
    )

    parser.add_argument(
        "--write_time_statistics", help="write time statistics", action="store_true"
    )

    # plot trajectories
    parser.add_argument(
        "--plot_trajectories", help="Plot the trajectories", action="store_true"
    )
    ...
    parser.add_argument(
        "--no_plot_traj_per_alg", action="store_false", dest="plot_traj_per_alg"
    )

    parser.add_argument(
        "--recalculate_errors", help="Deletes cached errors", action="store_true"
    )
    parser.add_argument(
        "--png", help="Save plots as png instead of pdf", action="store_true"
    )
    parser.add_argument("--dpi", type=int, default=300)
    parser.set_defaults(
        odometry_error_per_dataset=False,
    ...
    print(
        "Will analyze results from {0} and output will be "
    ...
    assert len(PALLETE) > len(algorithms), "Not enough colors for all configurations"
    algo_colors = {}
    ...
    eval_uid = "_".join(
        list(plot_settings["algo_labels"].values())
    ...
    for config_i in algorithms:
        ...
        cur_mulierror_i = []
        for d in datasets:
            print(Fore.RED + "--- Processing {0}-{1}... ---".format(config_i, d))
            exp_name = args.platform + "_" + config_i + "_" + d
            trace_dir = os.path.join(
                args.results_dir, args.platform, config_i, exp_name
            )
            ...
            mt_error.uid = "_".join([args.platform, config_i, d, str(n_trials)])
    ...
    if args.rmse_table:
        ...
        res_writer.write_tex_table(
            ...
            rmse_table["cols"],
            os.path.join(
                output_dir, args.platform + "_translation_rmse_" + eval_uid + ".txt"
            ),
        )

    if args.rmse_boxplot and n_trials > 1:
        rmse_plot_alg = [v for v in algorithms]
        algorithm_rmse = collect_rmse_per_dataset(config_multierror_list, rmse_plot_alg)
        print("--- Generate boxplot for RMSE ---")
        plot_rmse_per_dataset(
            algorithm_rmse, datasets, algorithms, output_dir, plot_settings
        )
    ...
    if args.odometry_error_per_dataset:
        ...
        print(Fore.MAGENTA + "--- Generating relative (KITTI style) error plots ---")
        plot_odometry_error_per_dataset(
            dataset_rel_err, datasets, algorithms, datasets_res_dir, plot_settings
        )
    ...
    if args.overall_odometry_error:
        ...
        for alg in algorithms:
            ...
            for et in rel_err_names:
                ...
                for _, v in all_odo_err[et][alg].items():
                    cur_errors.extend(v)
                alg_aver_rel.append(
                    "{0:.3f}".format(res_writer.compute_statistics(cur_errors)["mean"])
                )
        ...
        res_writer.write_tex_table(
            ...
            rel_err_table["rows"],
            rel_err_table["cols"],
            os.path.join(output_dir, args.platform + "_rel_err_" + eval_uid + ".txt"),
        )
    ...
    if args.write_time_statistics:
        ...
        for didx, d in enumerate(datasets):
            with open(
                os.path.join(datasets_res_dir[d], "{}_meas_stats.json".format(d)), "w"
            ) as f:
    ...
    s.call(
        [
            "notify-send",
            "rpg_trajectory_evaluation finished",
