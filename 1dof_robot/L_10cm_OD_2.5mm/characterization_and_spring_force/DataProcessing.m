...
% Output [px]: radius, curvature, arc_length, x_tip, y_tip = cv_output.csv
output = load('data/cv_output.csv');
radius = output(:,1);
radius = radius(round(start_ratio*length(radius)):end);

%curvature = output(:,2); not used
arc_length = output(:,3);
...
curvature = 1 ./ radius;
arc_length = 2*arc_length*conv_rate;
...
alpha_nominal = circle_model_curvature .* 2*L;
alpha_opt = opt_model_curvature .* 2*L;
