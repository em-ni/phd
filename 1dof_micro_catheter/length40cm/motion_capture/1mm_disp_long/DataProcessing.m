...
% Mouse clicked at 4 [s]
video_duration = 65; % [s] 
start_ratio = 4/video_duration;
...
%% FILTER DATA
lpFilt_e = designfilt('lowpassiir', 'PassbandFrequency', 0.3, 'StopbandFrequency', 5, 'PassbandRipple', 1, 'StopbandAttenuation', 20, 'SampleRate', fs); 
...
% Clean data at the extremes
pressure_f = pressure_f(2500:end-2000);
%curvature_f = curvature_f(100:end-100);
...
time_shifted = (1:length(pressure_f)) + 0;
...
cut_in = 1500;
cut_end = 2000;
t_new = t_new(cut_in:end-cut_end);
pressure_f = pressure_f(cut_in:end-cut_end);
curvature_f_resampled = curvature_f_resampled(cut_in:end-cut_end);
...
k_opt = 0.028; % Approximation to highlight differences
