...
class LungGraphBuilder:
    def __init__(self, vtk_path, cad_path):
        ...
        self.voxel_pitch = None  # Will be calculated dynamically
    ...
    def _compute_sdf_from_cad(self):
        ...
        extents = mesh.extents  # [x_len, y_len, z_len]
        max_dim = np.max(extents)
        
        # We want at least 256 voxels along the longest axis for good precision
    ...
    def visualize(self, save_screenshot="verification.png"):
        ...
        # Use off_screen=False if you have a display, True if purely headless
        p = pv.Plotter(off_screen=False) 

        # 1. Plot Original CAD (Increased opacity + Distinct Color)
        if os.path.exists(self.cad_path):
            mesh = pv.read(self.cad_path)
            # Use 'wheat' or 'grey' for better contrast with blue tubes
            p.add_mesh(mesh, color='wheat', opacity=0.25, label='CAD Wall')
        
        # 2. Plot Processed Nodes (Red Spheres)
        ...
        # 3. Plot Processed Edges (SDF Tubes)
        # Reduced opacity to 0.4 so we can see the skeleton inside
        tubes = []
        for u, v, data in self.final_graph.edges(data=True):
            pts = data['pts']
            line = pv.lines_from_points(pts)
            rad = np.mean(data['radius'])
            # Safety clamp for viz: don't let visualization tubes get massive if calculation errs
            if rad < 0.1: rad = 0.5 
        ...
        # 4. Plot Original VTK Skeleton (THICKER and LAST to appear on top)
        if os.path.exists(self.vtk_path):
            skel = pv.read(self.vtk_path)
            # line_width=4 makes it highly visible inside the semi-transparent tubes
...
if __name__ == "__main__":
    vtk_file = "centerline.vtk" 
    cad_file = "lungs.obj" # Make sure this path is correct
    
    builder = LungGraphBuilder(vtk_file, cad_file)
    G = builder.process()
    builder.export_npz("deep_lung_graph.npz")
    builder.visualize()